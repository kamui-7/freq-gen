...
class Files(QtCore.QThread):
    ...
    def gen_freq(self):
        freqTitle = os.path.basename(self.files[0].filepath)[:-4] + "_Frequency_List" + self.type
        counter = Counter(self.words)
        trashre = re.compile(r"^['\"{}!(:/.\\0-9%^&*@#$_\-~`|+•『』〔〕［］\[\]｛｝｟｠〈〉。、>< ,・=;※ー【】〖〗〘〙)a-zA-Z]{0,2}$")
        mc = counter.most_common()
        mc = {k: v for k, v in mc if not bool(re.findall(trashre, k))}
        freqinfo = [k for k, v in mc.items()]
        with open(os.path.join(self.out, freqTitle), "w",encoding="utf8") as freq:
            if self.type == ".csv":
                obj = csv.writer(freq)
                for num, row in enumerate(freqinfo):
                    if self.addoccur:
                ...
                if self.addoccur:
                    freq.write('\n'.join(['\t'.join([k, str(v)]) for k, v in counter.most_common()]))
                else:
                    freq.write('\n'.join(freqinfo))
...
class mw(Ui_Form):
    ...
    def setupUi(self):
        ...
        self.generate.clicked.connect(lambda: self.genfreq(self.filetype.currentText()))
    ...
    def choosedir(self):
        self.dir = str(QtWidgets.QFileDialog.getExistingDirectory(self,"Output Folder"))
        self.outpath.setText(self.dir)

    def genfreq(self, filetype):
        files = [str(self.filewidget.item(i).text()) for i in range(self.filewidget.count())] + [self.url.text()]
        ...
        files = [File(filename,self.lang.currentText()) for filename in files if filename]
        worker = Worker(files,self.dir,self.filetype.currentText(),self.addoccur.isChecked())
    ...
    def clearList(self):
        self.filewidget.setCurrentItem(self.filewidget.item(0))
        for i in range(self.filewidget.count()):
